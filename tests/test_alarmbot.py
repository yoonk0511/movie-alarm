...
def test_search_theaters_filters_by_query_substring(monkeypatch):
    ...
    monkeypatch.setattr(bot, "cgv_browser_session", fake_browser_session)
    monkeypatch.setattr(bot, "CgvApiClient", lambda page: fake_client)

    matches = run(bot.search_theaters("용산"))
...
def test_search_theaters_returns_empty_when_no_match(monkeypatch):
    ...
    assert run(bot.search_theaters("없는극장")) == []
...
def test_fetch_showtimes_for_site_uses_site_name_not_site_no(monkeypatch):
    ...
    monkeypatch.setattr(bot, "cgv_browser_session", fake_browser_session)
    monkeypatch.setattr(bot, "CgvTheaterClient", FakeTheaterClient)

    entries = run(bot.fetch_showtimes_for_site("용산아이파크몰", "20260810"))
...
def test_distinct_sorted_dedupes_and_sorts():
    ...
    assert bot._distinct_sorted(entries, "g") == ["4DX", "아이맥스"]
...
def test_describe_target_joins_multiple_dates_and_grades():
    ...
    assert bot.describe_target(target) == "F1 / 20260810, 20260811 / 4DX, 아이맥스"


def test_describe_target_defaults_when_empty():
    target = make_target(movie="", date=[], grades=[])
    assert bot.describe_target(target) == "전체 영화 / 전체 날짜 / 등급 무관"
...
def test_add_cmd_uses_site_name_not_site_no_for_showtime_lookup(monkeypatch):
    ...
    monkeypatch.setattr(bot, "fetch_showtimes_for_site", fetch_mock)
    monkeypatch.setattr(bot, "add_target", lambda *a, **k: (True, make_target()))
    interaction = make_interaction()
